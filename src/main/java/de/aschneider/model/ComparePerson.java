...
public class ComparePerson {
	...
	public static void main(String[] args) {
		...
		List<Person> peopleDescendingAge = people.stream().sorted(Person.comparePersonAgeDescending())
				.collect(Collectors.toList());
		...
		Map<Integer, String> map = people.stream().collect(
				Collectors.toMap(Person::getAge, Person::getName, (name1, name2) -> name1 + ";" + name2));

		System.out.println("People grouped by Age:" + map);
		// ------------------------------------------------------
		...
		// ------------------------------------------------------
		String phrase = people.stream().filter(person -> person.getAge() >= 18).map(Person::getName)
